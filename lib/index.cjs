...
/// TRANSACTIONS ////////////////////////////////////////////////////////////////////////////////////////////
    ...
    */
     ...
     *
     */
     async function getTransactionOutcome(blockchain, txID, timeoutSec) {
         ...
         const checkTransaction = async () => {
             ...
             try {
                 const data = await getTransactionbyID(blockchain, txID, 0, 10);
                 console.log('Data received:', data);
                 
                 if (data.Result === 200 && data.Response !== 'Transaction Not Found' && data.Response.Status !== 'Pending') {
                     return data.Response;  // Return if transaction is found and confirmed
                 } else {
                     console.log('Transaction not yet confirmed or not found, polling again...');
                     await new Promise(resolve => setTimeout(resolve, interval));  // Wait for the interval before polling again
                     return await checkTransaction();  // Recursive call
                 }
             } catch (error) {
         ...
         return await checkTransaction();  // Start polling immediately
     }
