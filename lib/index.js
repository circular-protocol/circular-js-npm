    ...
    */
    async function sendTransactionWithPK(from, senderPK, to, type, payload, blockchain) {
        try {
            ...
            // Await the result of the getWalletNonce call
            ...
            // Get the current timestamp and create the transaction ID
            let Timestamp = getFormattedTimestamp();
            let ID = sha256(blockchain + from + to + payload + String(Nonce) + Timestamp);
    
            // Sign the transaction ID using the sender's private key
            let Signature = signMessage(ID, senderPK); // Sender's private key is used for the signature
